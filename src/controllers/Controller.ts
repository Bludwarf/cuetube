...
/**
 * Fonction déclarée dans le HTML pour récupérer le controller Angular actuel
 */
declare function getCtrl();
...
export default class PlayerCtrl {
    static $inject = ['$scope', '$http', 'cuetubeConf'/*, '$ngConfirm'*/, 'gapiClient'];
    constructor(private $scope, private $http, private cuetubeConf/*, private $ngConfirm*/, private gapiClient) {
      ...
      if (isInit && persistence instanceof GoogleDrivePersistence) {
        ...
        $scope.onYouTubeIframeAPIReady = function () {
            ...
            if (currentStr) {
                ...
                const disc: Disc = _.find($scope.discs, (disc) => disc && disc.id === current.discId);
        ...
        $scope.loadTrackIndex = function (trackIndex, fileIndex, discIndex) {
            ...
            trackIndex = defaults(trackIndex, $scope.currentTrack && $scope.currentTrack.index);
            fileIndex = defaults(fileIndex, $scope.currentTrack && $scope.currentTrack.file.index);
            discIndex = defaults(discIndex, $scope.currentTrack && $scope.currentTrack.disc.index);
         ...
         */
        $scope.save = function () {
            alert("FIXME TypeScript"); return;// FIXME TypeScript :
