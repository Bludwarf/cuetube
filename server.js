...
router
    ...
    .get("/player", function(req, res) {
        ...
        const discsParam = req.query.discs;
        
        // Validater que au moins un paramètre est fourni
        if (!collection && !discsParam) {
            return res.status(400).json({ 
                error: 'Missing required parameter: collection or discs' 
            });
        }
        
        // Charger une collection si le paramètre collection est fourni
        if (collection) {
            const LocalAndDistantPersistence = require('./src/persistence/LocalAndDistantPersistence').LocalAndDistantPersistence;
            const persistence = new LocalAndDistantPersistence();
            
            return persistence.loadCollection(collection)
                .then(discs => {
                    res.json({
        ...
        // Charger des discs spécifiques si le paramètre discs est fourni
        if (discsParam) {
            const discIds = discsParam.split(',');
            const LocalAndDistantPersistence = require('./src/persistence/LocalAndDistantPersistence').LocalAndDistantPersistence;
            const persistence = new LocalAndDistantPersistence();
            
            // Charger chaque disc
            Promise.all(discIds.map(discId => persistence.loadDisc(discId)))
                .then(discs => {
                    res.json({
                        discs: discs.filter(d => d !== null) || []
                    });
                })
                .catch(err => {
    ...
    .get("/edit/:id.cue", function(req, res) {
        ...
        if (!discId || discId === '') {
            return res.status(400).json({ error: 'Missing disc ID' });
        }
        
        const LocalAndDistantPersistence = require('./src/persistence/LocalAndDistantPersistence').LocalAndDistantPersistence;
        const persistence = new LocalAndDistantPersistence();
        
        return persistence.loadDisc(discId)
            .then(disc => {
                if (!disc) {
    ...
    // Route pour gérer /edit/.cue (sans ID)
    .get("/edit/.cue", function(req, res) {
        return res.status(400).json({ error: 'Missing disc ID' });
    })
    .get("/edit-cue", function(req, res) {
        // Pour la compatibilité arrière, on peut garder la redirection ou envoyer un fichier
        const discId = req.query.id;
        if (discId) {
            res.redirect("/edit/" + discId + ".cue");
        } else {
    ...
    .get("/api/youtube/:videoId", function(req, res) {
        const videoId = req.params.videoId;
        const ytHelper = require('./src/yt-helper');
        
        ytHelper.fetchVideo(videoId)
            .then(video => {
                res.json(video);
            })
            .catch(err => {
                ...
                res.status(status).json({ 
                    error: err.message || 'Error fetching video' 
                });
            });
    })
    ...
    // Route pour tester le parsing JSON
...
// Ne pas démarrer le serveur en mode test - supertest gère cela
if (process.env.NODE_ENV !== 'test') {
